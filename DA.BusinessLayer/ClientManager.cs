...
namespace DA.BusinessLayer
{
    public class ClientManager
    {
        ...
        public IList<tbl_UserClientMapping> GetMappedClientDetails(int? userID)
        {
            try
            {
                IGenericDataRepository<tbl_UserClientMapping> repository = new GenericDataRepository<tbl_UserClientMapping>();
                IList<tbl_UserClientMapping> lstMappedClients = repository.GetList(c => c.UserID == userID);

                return lstMappedClients;
            }
            catch (Exception)
            {
                throw;
